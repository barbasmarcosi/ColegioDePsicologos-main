...
function MainProvider(props) {
  ...
  const localBillStg = (set) => {
    if (set) {
      localStorage.setItem("FACTURAS", JSON.stringify(set));
    } else {
  ...
  if (searchBill.length >= 1) {
    bills = bills.filter((bill) => {
      const billNumber = `${bill.nroFactura}`;
      const description = bill.descripcion.toLowerCase();
      const beginDate = bill.fechaFactura.toLowerCase();
      const amount = `${bill.monto}`;
      const finishDate = bill.fechaIngreso.toLowerCase();
      const cuit = `${bill.cuitMatriculado}`;
      const searchText = searchBill.toLowerCase();
      return (
  ...
  const addPerson = (cuit, nombre, apellido, fromFile) => {
    ...
    if (gridIndex !== -1 && persons[gridIndex].estado === 0) {
      ...
      persons[gridIndex].estado = 1;
      !fromFile && alert("Esta persona ya estaba registrada");
    } else {
      let person = {
  ...
  const addBill = (
    ...
    bills.sort(dynamicSort("id"));
    let gridIndex = -1;
    gridIndex = bills.findIndex(
      (bill) => Number(bill.nroFactura) === Number(nroFactura)
    );
    if (gridIndex !== -1 && bills[gridIndex].estado === 0) {
      alert(
        "Este numero de factura ya estaba cargada pero anulada, sera dada de alta con los datos originales"
      );
      bills[gridIndex].estado = 1;
    } else if (gridIndex !== -1 && bills[gridIndex].estado === 1) {
      alert("Este numero de factura ya estaba cargada");
    } else {
      let bill = {
  ...
  const addRetention = (
    ...
    if (gridIndex !== -1 && retentions[gridIndex].estado === 0) {
      ...
      !fromFile && alert("Esta retencion ya estaba cargada");
    } else {
      let retention = {
        id:
          retentions.length !== 0
  ...
  const addLiquidation = (mes, anio, cuitMatriculado, nombre) => {
    retentions.sort(dynamicSort("id"));
    let searchedBills = bills.filter((bill) => {
    ...
    if (searchedBills.length > 0) {
      ...
      let searchedRetention = retentions.filter((retention) => {
        ...
        const retentionYear = new Date(retention.fecha).getFullYear();
        const search = `${cuitMatriculado}`;
        console.log(retentionCuit, search,  retentionMonth , mes , retentionYear , anio);
        return retentionCuit.match(search) && retentionMonth === mes && retentionYear === anio;
      });
      console.log(searchedRetention);
  ...
  const modifyBill = (
    ...
    bills.sort(dynamicSort("id"));
    bills[id - 1].descripcion = descripcion;
    bills[id - 1].fechaFactura = fechaFactura;
    bills[id - 1].monto = monto;
    bills[id - 1].cuitMatriculado = cuitMatriculado;
    localBillStg(bills);
  };
  ...
  const deleteBill = (id) => {
    ...
    const gridIndex = bills.findIndex((bill) => bill.id === id);
    if (!bills[gridIndex].idLiquidacion) {
      bills[gridIndex].estado = 0;
      localBillStg(bills);
    } else {
  ...
  const deleteLiquidation = (id) => {
    ...
    bills.forEach((bill) => {
      if (bill.idLiquidacion === liquidations[gridIndex].id) {
        bill.idLiquidacion = null;
