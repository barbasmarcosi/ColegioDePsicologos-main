...
function AppUI() {
  ...
  const liquidationsHeader = [
    {
      id: 1,
      value: "Monto liquidado",
      column: "montoMes",
    },
    {
      id: 2,
  ...
  return (
    <React.Fragment>
      ...
      {selectedNav === 2 && (
        <>
          <Input search={searchBill} setSearch={setSearchBill} />
          <AddButton setModal={setOpenAddBillModal}>Cargar Factura</AddButton>
          <GridContainer>
            <thead>
              <tr>
                {billHeader.map((header) => (
                  <HeadGrid
                    value={header.value}
                    column={header.column}
                    id={header.id}
            ...
            <tbody>
              {bills.map((project) => (
                <BillBodyGrid
                  ...
                  onDelete={() => deleteBill(project.id)}
                ></BillBodyGrid>
              ))}
            </tbody>
          </GridContainer>
      ...
      {!!loading && <Loading><h1>Esepere por favor...</h1></Loading>}
    </React.Fragment>
  );
}
